...
# --- ここからフォールバック（google_api_utils が無い場合） ---
if get_google_credentials is None or create_google_doc_external is None:
    ...
    def get_google_credentials():
        # secrets.toml に GOOGLE_CREDENTIALS を入れる運用に合わせる
        raw = st.secrets["GOOGLE_CREDENTIALS"]
        info = json.loads(raw) if isinstance(raw, str) else raw
        creds = service_account.Credentials.from_service_account_info(info)
        # 必要に応じてスコープを付与（Docs/Drive）
        scopes = [
    ...
    def create_google_doc_external(book_title: str, chapter_title: str, text: str, creds, root_name: str = "OCR結果"):
        if build is None:
            raise ImportError("googleapiclient が必要です。`pip install google-api-python-client`")
        docs = build("docs", "v1", credentials=creds)
        drive = build("drive", "v3", credentials=creds)

        # フォルダ用意: OCR結果/書籍タイトル
        book_folder_id = _ensure_folder_path(drive, [root_name, book_title])
        ...
        # 章タイトル=見出し2 + 本文（既に本文先頭に "## 章" がある場合は重複を避ける）
        heading = chapter_title.strip() or "無題"
        body_text = text or ""
        first_line = (body_text.splitlines() or [""])[0].strip()
        if first_line == f"## {heading}":
        ...
        drive.files().update(fileId=doc_id, addParents=book_folder_id, removeParents=prev_parents, fields="id, parents").execute()
        return doc_id
# --- フォールバックここまで ---
